...
Motion Atlas 是可直接交付的实现基准。没有额外要求时，AI 会使用同一份 canonical profile，让节奏、距离、错峰、光感和降级策略与演示保持一致。

1:1 复现不等于锁死参数。可以直接用自然语言让 AI 微调速度、移动或浮动幅度、扫光覆盖、尺寸、密度、分区和空间拓扑；AI 会保留该 Motion 的核心意图、视觉层级和品质红线，只调整明确提出的部分。

> 保持 06 Shimmer Sweep 的整体质感，把扫光放慢 15%，扩大覆盖范围，并进一步收住亮度。

> 保留 10 Orbit Network 的层级关系，把平面轨道改成三个相互关联的分区，再尝试球形投影。

SU Motion Top 12 面向偏好克制、精致、清楚且有秩序的动效表达。它让运动融入产品、帮助内容，而不是抢走内容的注意力。
...
- **06 完整扫过**：修复重复位移导致的有效扫光过短；新的光带从边缘进入、完整穿过材质并离开，峰值亮度更克制。
- **10 可换拓扑**：Atlas 继续展示平面轨道，同时实现支持分区聚类与球形投影的拓扑策略，后续可以扩展布局而不改变 Motion 10 的身份。
- **1:1 但不锁死**：canonical profile 仍是默认交付基准；用户可以让 AI 微调明确参数，未提出的结构和品质红线保持不变。
- **新增回归保护**：自动检查 06 的覆盖宽度与亮度上限，以及 10 的拓扑扩展能力和 Canvas 分辨率上限。
...
- **Observed in real websites**：目录来自本地真实网页与动效盘点，不是凭空命名。
